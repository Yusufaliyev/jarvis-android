...
class _JarvisScreenState extends State<JarvisScreen>
  ...
  Future<void> _listen() async {
    ...
    if (!status.isGranted) {
      _speak('Mikrofon ruxsati kerak! Sozlamalardan ruxsat bering.');
      return;
    }
    ...
    bool ok = await _speech.initialize(
      onError: (e) {
        ...
        _speak('Mikrofon xatosi. Qayta urinib ko\'ring.');
      },
    );

    if (!ok) {
      _speak('Mikrofon ishga tushmadi!');
      return;
    }
    ...
    _speech.listen(
      onResult: (r) {
        ...
        if (r.finalResult && _userText.isNotEmpty) {
          _process(_userText);
        }
      },
      localeId: 'uz_UZ',
  ...
  Future<void> _process(String text) async {
    ...
    // Ob-havo alohida tekshirish
    if (text.toLowerCase().contains('havo') ||
        text.toLowerCase().contains('ob-havo') ||
        text.toLowerCase().contains('temperatura')) {
      String city = 'Toshkent';
      if (text.toLowerCase().contains('samarqand')) city = 'Samarkand';
      if (text.toLowerCase().contains('buxoro')) city = 'Bukhara';
      final weather = await WeatherService.getWeather(city);
      setState(() => _thinking = false);
      await _speak(weather);
    } else {
      ...
      children: [
        ...
        Row(children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              border: Border.all(
                  color: Color(0xFF00FF88).withOpacity(0.35)),
  ...
  Widget _textBox() {
    ...
    final bc = _listening
        ? Color(0xFFFF3366)
        : _thinking ? Color(0xFFFFAA00) : Color(0xFF00D4FF);
    ...
    return AnimatedSwitcher(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: bc.withOpacity(0.05), blurRadius: 20)],
        ),
        child: Text(txt,
          style: TextStyle(
