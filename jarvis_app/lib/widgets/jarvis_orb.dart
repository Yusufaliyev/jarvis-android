...
class OrbPainter extends CustomPainter {
  ...
  OrbPainter({required this.rotation, required this.pulse, required this.particle,
    required this.isListening, required this.isThinking, required this.isSpeaking});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(c, r, Paint()
      ..shader = RadialGradient(colors: [
        ...
        color.withOpacity(0.08 + pulse * 0.08),
        color.withOpacity(0.0),
      ], stops: [0.4, 0.7, 1.0]).createShader(Rect.fromCircle(center: c, radius: r)));
    ...
    canvas.drawCircle(c, coreR, Paint()
      ..shader = RadialGradient(colors: [
        ...
        Colors.transparent,
      ], stops: [0.0, 0.25, 0.6, 1.0]).createShader(Rect.fromCircle(center: c, radius: coreR)));

    // Ichki halqa
    canvas.drawCircle(c, coreR + 4, Paint()
      ..color = color.withOpacity(0.3 + pulse * 0.3)
      ..style = PaintingStyle.stroke
      ..strokeWidth = 1.0);

    // To'lqin (gapirsa)
    if (isSpeaking || isListening) _wave(canvas, c, r, pulse);
  }
  ...
  void _neural(Canvas canvas, Offset c, double r, double rot) {
    ...
    for (int i = 0; i < pts.length; i++) {
      ...
      canvas.drawLine(pts[i], c, p..color = color.withOpacity(0.07));
    }
  }
  ...
  void _particles(Canvas canvas, Offset c, double r, double rot) {
    ...
    for (int i = 0; i < 14; i++) {
      ...
      canvas.drawCircle(
        Offset(c.dx + cos(a) * rad, c.dy + sin(a) * rad * 0.4),
        1.5 + rng.nextDouble() * 2.5,
        Paint()..color = color.withOpacity(0.35 + rng.nextDouble() * 0.5),
      );
    }
  }

  void _wave(Canvas canvas, Offset c, double r, double p) {
    for (int i = 1; i <= 3; i++) {
      canvas.drawCircle(c, r * 0.35 + i * 15 + p * 10, Paint()
        ..color = color.withOpacity((0.3 - i * 0.08) * p)
        ..style = PaintingStyle.stroke
        ..strokeWidth = 1.0);
    }
  }
